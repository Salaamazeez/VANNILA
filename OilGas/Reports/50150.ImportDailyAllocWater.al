report 50150 ImportDialyAllocWater
{
    ...
    dataset
    {
        dataitem(Integer; "Integer")
        {
            ...
            begin
                ImportSheet(Number);
                WindowDialog.Update(1, DailyWaterAllocation.Well);
                WindowDialog.Update(2, DailyWaterAllocation."Production Date");

                DailyWaterAllocation.Init();
                Evaluate(DailyWaterAllocation."Production Date", ColText[1]);
                DailyWaterAllocation.Facility := ColText[2];
                DailyWaterAllocation.Fields := ColText[3];
                DailyWaterAllocation.OML := ColText[4];
                DailyWaterAllocation.Well := ColText[5];
                DailyWaterAllocation."Well Type" := ColText[6];
                Evaluate(DailyWaterAllocation."Daily Allocated", ColText[7]);
                IF DailyWaterAllocation.INSERT(True) then begin
                    RecordCount += 1;
                end ELSE begin
                    DailyWaterAllocation.Modify();
                    RecordModified += 1;
                end;
    ...
    trigger OnPreReport()
    begin
        WindowDialog.Open(TextDisplay, DailyWaterAllocation.Well, DailyWaterAllocation."Production Date");
    end;
    ...
    var
        ...
        DailyWaterAllocation: Record DailyAllocationWater;
